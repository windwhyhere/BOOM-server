  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-2 mb-2">
         <span className="text-xs font-bold text-indigo-600 bg-indigo-50 px-2 py-1 rounded-md border border-indigo-100 uppercase tracking-wide">
      ...
      <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-200">
        <div className="flex items-center justify-between pb-4 border-b border-slate-100 mb-4">
            ...
            <div>
                <h3 className="font-semibold text-slate-800">通知系统总开关</h3>
                <p className="text-xs text-slate-500">控制所有下发通知的全局开关</p>
            </div>
            </div>
            <Switch 
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {/* FCM Switch */}
            <div className={`flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-slate-100 transition-opacity ${isGlobalDisabled ? 'opacity-50 pointer-events-none' : ''}`}>
                <div className="flex items-center gap-2">
                    <Radio size={16} className={config.fcm.enabled ? "text-blue-500" : "text-slate-400"} />
                    <span className={`text-sm font-medium ${config.fcm.enabled ? "text-slate-700" : "text-slate-400"}`}>FCM 远程推送</span>
                </div>
            ...
            <div className={`flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-slate-100 transition-opacity ${isGlobalDisabled ? 'opacity-50 pointer-events-none' : ''}`}>
                <div className="flex items-center gap-2">
                    <Clock size={16} className={config.local.enabled ? "text-emerald-500" : "text-slate-400"} />
                    <span className={`text-sm font-medium ${config.local.enabled ? "text-slate-700" : "text-slate-400"}`}>Local 本地通知</span>
                </div>
            ...
            <div className={`flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-slate-100 transition-opacity ${isGlobalDisabled ? 'opacity-50 pointer-events-none' : ''}`}>
                <div className="flex items-center gap-2">
                    <PlayCircle size={16} className={config.media.enabled ? "text-purple-500" : "text-slate-400"} />
                    <span className={`text-sm font-medium ${config.media.enabled ? "text-slate-700" : "text-slate-400"}`}>Media 媒体通知</span>
                </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="flex gap-2 p-1 bg-slate-200/50 rounded-xl w-fit">
            <button 
              ...
              className={`px-4 py-1.5 rounded-lg text-sm font-medium transition-all ${activeTab === 'remote' ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
            >
              FCM 远程
            </button>
            <button 
              onClick={() => setActiveTab('local')}
              className={`px-4 py-1.5 rounded-lg text-sm font-medium transition-all ${activeTab === 'local' ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
            >
              Local 本地
            </button>
            <button 
              onClick={() => setActiveTab('media')}
              className={`px-4 py-1.5 rounded-lg text-sm font-medium transition-all ${activeTab === 'media' ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
            >
              Media 媒体
            </button>
          </div>
          ...
          {activeTab === 'remote' && (
            <Card title="FCM Topic 策略配置" className="space-y-0" action={
                 <div className="text-xs text-slate-400 flex items-center gap-1">
                   {currentTopic ? <span className="font-mono">{currentTopic.type === 'data' ? 'DataMsg' : 'Notification'}</span> : ''}
                 </div>
            }>
               {/* Global FCM Settings */}
               <div className="grid grid-cols-1 md:grid-cols-1 gap-4 mb-6 pb-6 border-b border-slate-100">
                 <div className="md:col-span-1">
                   <TextArea 
                      label="Firebase Key (JSON)" 
                      rows={1} 
                      value={config.fcm.jsonConfig} 
                      onChange={(e) => setConfig({...config, fcm: {...config.fcm, jsonConfig: e.target.value}})}
               ...
               <div className="flex flex-col md:flex-row gap-6 min-h-[400px]">
                  ...
                  {/* Left: Topic List */}
                  <div className="w-full md:w-1/3 border-r border-slate-100 pr-0 md:pr-4 flex flex-col">
                     <div className="flex items-center justify-between mb-3">
                        <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">Topics</span>
                     ...
                     <div className="space-y-2 flex-1 overflow-y-auto max-h-[400px]">
                        {config.fcm.topics.map(topic => (
                          <div 
                             ...
                             className={`p-3 rounded-lg border cursor-pointer transition-all relative group ${selectedTopicId === topic.id ? 'bg-indigo-50 border-indigo-200 shadow-sm' : 'bg-white border-slate-100 hover:border-indigo-100'}`}
                          >
                             <div className="flex items-center gap-2 mb-1">
                                <Tag size={14} className={selectedTopicId === topic.id ? 'text-indigo-600' : 'text-slate-400'} />
                                <span className={`font-medium text-sm truncate ${selectedTopicId === topic.id ? 'text-indigo-900' : 'text-slate-600'}`}>
                                  {topic.name}
                                </span>
                             </div>
                             <div className="text-[10px] text-slate-400 pl-6 flex flex-col gap-0.5">
                             ...
                             <button 
                                ...
                                className={`absolute right-2 top-3 text-slate-300 hover:text-red-500 transition-colors ${selectedTopicId === topic.id ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}
                             >
                                <Trash2 size={14} />
                             </button>
                          </div>
                  ...
                  <div className="w-full md:w-2/3 pl-0 md:pl-2">
                     {currentTopic ? (
                        <div className="space-y-4 animate-fade-in flex flex-col h-full">
                           <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 space-y-4">
                             <Input 
                                ...
                                onChange={(e) => updateTopic(currentTopic.id, 'name', e.target.value)}
                                className="font-mono text-sm font-semibold text-indigo-900"
                             />
                             <div className="grid grid-cols-3 gap-4">
                                <Select 
                                    ...
                                    onChange={(e) => updateTopic(currentTopic.id, 'type', e.target.value)}
                                />
                                <Select 
                                    label="优先级" 
                                    options={[{label: 'High', value: 'high'}, {label: 'Normal', value: 'normal'}, {label: 'Low', value: 'low'}]}
                                    value={currentTopic.priority}
                                    onChange={(e) => updateTopic(currentTopic.id, 'priority', e.target.value)}
                                />
                                <Input 
                                    label="发送间隔 (mins)" 
                                    type="number" 
                                    value={currentTopic.intervalMins}
                                    onChange={(e) => updateTopic(currentTopic.id, 'intervalMins', parseInt(e.target.value))}
                                />
                             </div>
                           </div>

                           <div className="flex items-center justify-between mb-2">
                              <span className="text-xs font-bold text-slate-500 uppercase">文案策略池 ({currentTopic.strategies.length})</span>
                           ...
                           <div className="space-y-3 overflow-y-auto max-h-[450px] pr-2">
                              {currentTopic.strategies.map((strategy, index) => (
                                 <div key={strategy.id} className="relative p-4 rounded-xl border border-slate-200 hover:border-indigo-200 transition-colors bg-white group flex gap-3 items-start">
                                    {/* Index Badge */}
                                    <div className="w-6 h-6 rounded-full bg-slate-50 text-slate-500 flex items-center justify-center text-xs font-bold border border-slate-200 shrink-0 mt-1">
                                      {index + 1}
                                    </div>
                                    ...
                                    <div className="space-y-2 flex-1">
                                       <Input 
                                          ...
                                          onChange={(e) => updateStrategy(currentTopic.id, strategy.id, 'title', e.target.value)}
                                       ...
                                       <TextArea 
                                          ...
                                          rows={2}
                                          value={strategy.body}
                                          onChange={(e) => updateStrategy(currentTopic.id, strategy.id, 'body', e.target.value)}
                                       ...
                                       <div className="flex gap-2">
                                          <div className="flex-1">
                                            <Input 
                                                ...
                                                onChange={(e) => updateStrategy(currentTopic.id, strategy.id, 'imageUrl', e.target.value)}
                                                className="text-xs text-slate-500"
                                            />
                                          </div>
                                          {strategy.imageUrl && (
                                            <div className="w-8 h-8 rounded bg-slate-100 shrink-0 overflow-hidden border border-slate-200">
                                              <img src={strategy.imageUrl} className="w-full h-full object-cover" alt="preview" />
                                            </div>
                                          )}
                                       </div>
                                    </div>
                                    <button 
                                      ...
                                      className="text-slate-300 hover:text-red-500 hover:bg-red-50 p-1.5 rounded-md transition-all opacity-0 group-hover:opacity-100 self-start"
                                    >
                                      <Trash2 size={14} />
                                    </button>
                                 </div>
                        ...
                        <div className="h-full flex flex-col items-center justify-center text-slate-400">
                          ...
                          <p className="text-sm">请在左侧选择或新建 Topic</p>
                        </div>
                     )}
                  </div>
               </div>
          ...
          {activeTab === 'local' && (
            <Card title="本地通知 (按 Channel)" className="space-y-5">
               <div className="flex flex-col md:flex-row gap-6 min-h-[400px]">
                     ...
                     <div className="space-y-2 flex-1 overflow-y-auto max-h-[400px]">
                        {config.local.channels.map(channel => (
                          <div 
                             ...
                             className={`p-3 rounded-lg border cursor-pointer transition-all relative group ${selectedChannelId === channel.id ? 'bg-indigo-50 border-indigo-200 shadow-sm' : 'bg-white border-slate-100 hover:border-indigo-100'}`}
                          >
                             <div className="flex items-center gap-2 mb-1">
                                <Clock size={14} className={selectedChannelId === channel.id ? 'text-indigo-600' : 'text-slate-400'} />
                                <span className={`font-medium text-sm truncate ${selectedChannelId === channel.id ? 'text-indigo-900' : 'text-slate-600'}`}>
                                  {channel.name}
                                </span>
                             </div>
                             <div className="text-[10px] text-slate-400 pl-6 flex flex-col gap-0.5">
                             ...
                             <button 
                                ...
                                className={`absolute right-2 top-3 text-slate-300 hover:text-red-500 transition-colors ${selectedChannelId === channel.id ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}
                             >
                                <Trash2 size={14} />
                             </button>
                          </div>
                  ...
                  <div className="w-full md:w-2/3 pl-0 md:pl-2">
                     {currentChannel ? (
                        <div className="space-y-5 animate-fade-in flex flex-col h-full">
                           <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 space-y-4">
                              <Input 
                                 ...
                                 onChange={(e) => updateChannel(currentChannel.id, 'name', e.target.value)}
                              ...
                              <div className="grid grid-cols-2 gap-4">
                                <Input 
                                  ...
                                  onChange={(e) => updateChannel(currentChannel.id, 'firstSendDelayMins', parseInt(e.target.value))}
                                />
                                <Input 
                                  label="重复频次 (分钟)" 
                                  type="number" 
                                  value={currentChannel.intervalMins}
                                  onChange={(e) => updateChannel(currentChannel.id, 'intervalMins', parseInt(e.target.value))}
                                />
                              </div>
                           </div>
                           
                           <div className="flex items-center justify-between mb-2 mt-4">
                              <span className="text-xs font-bold text-slate-500 uppercase">文案策略池 ({currentChannel.strategies.length})</span>
                                 ...
                                 <div key={strategy.id} className="relative p-4 rounded-xl border border-slate-200 hover:border-indigo-200 transition-colors bg-white group flex gap-3 items-start">
                                    ...
                                    <div className="space-y-2 flex-1">
                                       <Input 
                                          placeholder="标题" 
                                          value={strategy.title}
                                          onChange={(e) => updateLocalStrategy(currentChannel.id, strategy.id, 'title', e.target.value)}
                                          className="font-bold text-sm"
                                       />
                                       <TextArea 
                                          placeholder="详情内容" 
                                          rows={2}
                                          value={strategy.body}
                                          onChange={(e) => updateLocalStrategy(currentChannel.id, strategy.id, 'body', e.target.value)}
                                          className="text-sm resize-none"
                                       />
                                       <div className="flex gap-2">
                                          <div className="flex-1">
                                            <Input 
                                                placeholder="图片 URL (可选)" 
                                                value={strategy.imageUrl}
                                                onChange={(e) => updateLocalStrategy(currentChannel.id, strategy.id, 'imageUrl', e.target.value)}
                                                className="text-xs text-slate-500"
                                            />
                                          </div>
                                          {strategy.imageUrl && (
                        ...
                        <div className="h-full flex flex-col items-center justify-center text-slate-400">
                          <Layers size={32} className="mb-2 opacity-50"/>
                          <p className="text-sm">请在左侧选择或新建 Channel</p>
                        </div>
                     )}
                  </div>
               </div>
          ...
          {activeTab === 'media' && (
            <Card title="媒体通知 (按 Channel)" className="space-y-5">
               <div className="flex flex-col md:flex-row gap-6 min-h-[400px]">
                  
                  {/* Left: Channel List */}
                  <div className="w-full md:w-1/3 border-r border-slate-100 pr-0 md:pr-4 flex flex-col">
                     <div className="flex items-center justify-between mb-3">
                        <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">Media Channels</span>
                     ...
                     <div className="space-y-2 flex-1 overflow-y-auto max-h-[400px]">
                        {config.media.channels.map(channel => (
                          <div 
                             ...
                             className={`p-3 rounded-lg border cursor-pointer transition-all relative group ${selectedMediaChannelId === channel.id ? 'bg-indigo-50 border-indigo-200 shadow-sm' : 'bg-white border-slate-100 hover:border-indigo-100'}`}
                          >
                             <div className="flex items-center gap-2 mb-1">
                                <PlayCircle size={14} className={selectedMediaChannelId === channel.id ? 'text-indigo-600' : 'text-slate-400'} />
                                <span className={`font-medium text-sm truncate ${selectedMediaChannelId === channel.id ? 'text-indigo-900' : 'text-slate-600'}`}>
                                  {channel.name}
                                </span>
                             </div>
                             <div className="text-[10px] text-slate-400 pl-6 flex flex-col gap-0.5">
                             ...
                             <button 
                                ...
                                className={`absolute right-2 top-3 text-slate-300 hover:text-red-500 transition-colors ${selectedMediaChannelId === channel.id ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}
                             >
                                <Trash2 size={14} />
                             </button>
                          </div>
                        ...
                        <div className="space-y-5 animate-fade-in flex flex-col h-full">
                           <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 space-y-4">
                              <Input 
                                 ...
                                 onChange={(e) => updateMediaChannel(currentMediaChannel.id, 'name', e.target.value)}
                                 className="font-semibold"
                              />
                              <div className="grid grid-cols-2 gap-4">
                                <Input 
                                  ...
                                  onChange={(e) => updateMediaChannel(currentMediaChannel.id, 'firstSendDelayMins', parseInt(e.target.value))}
                                />
                                <Input 
                                  label="重复频次 (分钟)" 
                                  type="number" 
                                  value={currentMediaChannel.intervalMins}
                                  onChange={(e) => updateMediaChannel(currentMediaChannel.id, 'intervalMins', parseInt(e.target.value))}
                                />
                              </div>
                           </div>
                           
                           <div className="flex items-center justify-between mb-2 mt-4">
                              <span className="text-xs font-bold text-slate-500 uppercase">文案策略池 ({currentMediaChannel.strategies.length})</span>
                                 ...
                                 <div key={strategy.id} className="relative p-4 rounded-xl border border-slate-200 hover:border-indigo-200 transition-colors bg-white group flex gap-3 items-start">
                                    ...
                                    <div className="space-y-2 flex-1">
                                       <Input 
                                          placeholder="标题" 
                                          value={strategy.title}
                                          onChange={(e) => updateMediaStrategy(currentMediaChannel.id, strategy.id, 'title', e.target.value)}
                                          className="font-bold text-sm"
                                       />
                                       <TextArea 
                                          placeholder="详情内容" 
                                          rows={2}
                                          value={strategy.body}
                                          onChange={(e) => updateMediaStrategy(currentMediaChannel.id, strategy.id, 'body', e.target.value)}
                                          className="text-sm resize-none"
                                       />
                                       <div className="flex gap-2">
                                          <div className="flex-1">
                                            <Input 
                                                placeholder="图片 URL (可选)" 
                                                value={strategy.imageUrl}
                                                onChange={(e) => updateMediaStrategy(currentMediaChannel.id, strategy.id, 'imageUrl', e.target.value)}
                                                className="text-xs text-slate-500"
                                            />
                                          </div>
                                          {strategy.imageUrl && (
        ...
        <div className="space-y-6">
          <Card title="预览">
            <div className="bg-slate-900 rounded-[2rem] p-3 border-4 border-slate-800 shadow-2xl mx-auto w-64">
              <div className="bg-white rounded-[1.5rem] overflow-hidden h-96 relative">
                 {/* Fake Status Bar */}
                 <div className="h-6 bg-slate-100 flex justify-between px-4 items-center text-[10px] text-slate-500 font-bold">
                    <span>9:41</span>
                    <span>5G</span>
                 </div>
                 ...
                 <div className="absolute top-2 left-2 right-2 bg-white/90 backdrop-blur-md shadow-lg rounded-2xl p-3 border border-slate-100 z-10 transition-all duration-300">
                    <div className="flex gap-3">
                       <div className="w-10 h-10 bg-primary-500 rounded-xl flex items-center justify-center shrink-0 shadow-sm">
                         {activeTab === 'media' ? <PlayCircle className="text-white w-5 h-5" /> : <Bell className="text-white w-5 h-5" />}
                       </div>
                       <div className="flex-1 min-w-0">
                         <div className="flex justify-between items-start">
                           <p className="font-bold text-xs text-slate-900">{selectedApp.name}</p>
                           <span className="text-[10px] text-slate-400">now</span>
                         </div>
                         <div className="mt-1">
                            <p className="text-xs font-bold text-slate-800 line-clamp-1">{previewData.title || '通知标题'}</p>
                            <p className="text-xs text-slate-600 line-clamp-2 leading-tight mt-0.5">
                    ...
                    {previewData.image && (
                      <div className="mt-2 h-24 bg-slate-100 rounded-lg overflow-hidden relative">
                        <img 
            ...
            <div className="mt-6 text-center space-y-2">
              <Button variant="secondary" className="w-full">发送测试消息</Button>
              {activeTab === 'remote' && currentTopic && (
                 <p className="text-[10px] text-slate-400">
                   当前展示 Topic: <span className="font-mono text-slate-600">{currentTopic.name}</span> (Strategy 1/{currentTopic.strategies.length})
                 </p>
              )}
               {activeTab === 'local' && currentChannel && (
                 <p className="text-[10px] text-slate-400">
                   当前展示 Channel: <span className="font-mono text-slate-600">{currentChannel.name}</span> (Strategy 1/{currentChannel.strategies.length})
                 </p>
              )}
               {activeTab === 'media' && currentMediaChannel && (
                 <p className="text-[10px] text-slate-400">
                   当前展示 Channel: <span className="font-mono text-slate-600">{currentMediaChannel.name}</span> (Strategy 1/{currentMediaChannel.strategies.length})
                 </p>
              )}
            </div>
          </Card>
